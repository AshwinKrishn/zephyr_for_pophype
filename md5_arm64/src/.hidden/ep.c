...
int main() 
{
  ...
  FILE *fp;
  char arr[] = "Ashwin";
  struct device * ivshmem_device = 0x00000000; 
...
*/
 ...
 ivshmem_device = device_get_binding(CONFIG_IVSHMEM_DEV_NAME);
 printf("ivshmem_device value is %x\n",ivshmem_device);
 
 void * shm_addr ;
 uint64_t sizeof_shrmem = ivshmem_get_mem(ivshmem_device,&shm_addr); 
 printf("Address of the pointer is %x\n",shm_addr);
 uint32_t * shared_area = (uint32_t*) shm_addr;
 printf("Writing data into shm\n\n");

 for(int i = 0 ; i < 100 ; i++)
 {
	*((char*)shared_area + i) = 'a';
 }
 printf("We are printing the contents of first 100 bytes of ivshmem:\n");
 for(int i = 0 ; i < 100 ; i++)
 {
	printf("%c",*((char*)shared_area + i));
 }
  return 0;
}
