...
volatile struct shared_meta const * shared_struct = 0x30100000 - 0x100;
...
void * shr_addr = 0x30000000;

uint32_t * common_addr  = (char * )shr_addr + 0x100000 -  0x100;

void * self_malloc(uint32_t size){
...
*/	
        ...
        *((uint32_t*)rw_buf.write_area) = 0x00000000 ;
        ...
        memset(shr_addr,'A',5120);
    	memset((char*)shr_addr  + 5120,'A',3072 + 1024 );
        for(int i = 0 ; i < 0x100000 ; i++)
        {
        	*((volatile char*)shr_addr + i)  = 'e' ;
		 
		if(i == 0x100000 -1)
		{
			printf("\nWriting last byte %d",i);
		}
	}
        for(int i = 0 ; i < 5120 + 2048 + 1024 ; i++)
        {
		if(i%2 == 0)
