...
void  offload_list_load(void * mylist )
{
	...
	struct offload_struct  ofld_vranlc ;
        ...
        ofld_vranlc.args[0].location = mylist ;

        ofld_vranlc.type = OFFLOAD_LIST;

        memcpy((void*)rw_buf.write_area , (void*)&ofld_vranlc , sizeof(struct offload_struct) );
        printf("Copied the sturct details for list modifications \n");
//	wait_for_other_core();	
